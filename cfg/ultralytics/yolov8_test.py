...
def parse_arguments():
    ...
    parser.add_argument("--test_mode", type=str, default='', help="测试实验模式")
    ...
    parser.add_argument("--max_det", type=int, default=100, help="最大检测数量")
...
def test(args):
    ...
    test_speed = False  # TODO 是否计算测试时间

    # 实验路径构建
    exp_name = f'{model_name}{model_stru}{model_cfg}{iou_type}{album}{img_size}'  # yolov8n-p2_cbam_siou_aug2_1280
    ...
    # 测试配置文件路径
    best_model_path = f'runs/ultralytics/{test_mode}/{exp_name}/train/weights/best.pt'
    ...
    for data_type in ['test_synth', 'test_real_nominal', 'test_real_edge']:
        ...
        model.val(
            ...
            project=project,
            name=f'{test_mode}/{exp_name}/test/{data_type}',
            exist_ok=True,
            # 自定义参数
    ...
    for data_type in ['test_synth', 'test_real_nominal', 'test_real_edge']:
        ...
        path_prediction = f'runs/ultralytics/{test_mode}/{exp_name}/test/{data_type}/predictions.json'
        path_prediction_modified = f'runs/ultralytics/{test_mode}/{exp_name}/test/{data_type}/predictions_modified.json'
    ...
    # 保存评价指标
    path_results = f'results/tables/metrics/metrics_{test_mode}_{exp_name}.csv'
