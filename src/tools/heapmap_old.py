...
from pytorch_grad_cam import GradCAMPlusPlus, GradCAM, XGradCAM  # noqa
...
def letterbox(
        im, new_shape=(640, 640), color=(114, 114, 114), auto=True,
...
class yolov8_heatmap:
    ...
    def __init__(self, weight, cfg, device, method, layer, backward_type, conf_threshold, ratio):
        ...
        colors = np.random.uniform(0, 255, size=(
            len(model_names), 3)).astype(np.int32)
    ...
    def __call__(self, img_path, save_path):
        ...
        tensor = torch.from_numpy(np.transpose(
            img, axes=[2, 0, 1])).unsqueeze(0).to(self.device)
        ...
        for i in trange(int(post_result.size(0) * self.ratio)):
            ...
            if self.backward_type == 'class':
                ...
                gradients = grads.gradients[0] + grads.gradients[1] + \
                    grads.gradients[2] + grads.gradients[3]
            else:
                gradients = grads.gradients[0] + grads.gradients[1] + \
                    grads.gradients[2] + \
                    grads.gradients[3] + grads.gradients[4]
            ...
            saliency_map = cv2.resize(
                saliency_map, (tensor.size(3), tensor.size(2)))
            ...
            cam_image = show_cam_on_image(
                img.copy(), saliency_map, use_rgb=True)
            cam_image = self.draw_detections(post_boxes[i], self.colors[int(post_result[i, :].argmax(
            ))], f'{self.model_names[int(post_result[i, :].argmax())]} {float(post_result[i].max()):.2f}', cam_image)
...
def get_params():
    params = {
        'weight': 'runs/detect/yolov8n/train/weights/best.pt',
        'cfg': 'yolov8n.yaml',
        'device': 'cuda:0',
...
if __name__ == '__main__':
    ...
    path_image = 'datasets/lard/YoloFormat/test_real_nominal/images/3s1eHvzkxxI_097.png'
    model = yolov8_heatmap(**get_params())
    model(path_image, 'results/figs/heatmap')

# ZBAA_36R_35_28.jpeg GradCAMPlusPlus 8 all
# YMLT_32L_35_25.jpeg GradCAMPlusPlus 8 all
# CYVR_26R_35_21.jpeg GradCAMPlusPlus 8 all
# FTTJ_5_35_24.jpeg GradCAMPlusPlus 8 all
# 3s1eHvzkxxI_097.png GradCAMPlusPlus 8 all
