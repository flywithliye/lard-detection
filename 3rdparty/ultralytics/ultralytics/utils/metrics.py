...
class WIoU_Scale:
    ...
    def _scaled_loss(cls, self, gamma=1.9, delta=3):
        if isinstance(self.monotonous, bool):
            if self.monotonous:
                return (self.iou.detach() / self.iou_mean).sqrt()  # v2
            else:
                beta = self.iou.detach() / self.iou_mean
                alpha = delta * torch.pow(gamma, beta - delta)  # v3
                return beta / alpha
        return 1


def bbox_iou(box1, box2, xywh=True, feat_diag2=None,
             GIoU=False, DIoU=False, CIoU=False,
             SIoU=False, EIoU=False, WIoU=False,
             MDPIoU1=False, MDPIoU2=False, ShapeIoU=False, 
             NWD=False, using_innerIoU=False, ratio=1,
             Focal=False, alpha=1, gamma=0.5, scale=False, 
             shape_iou_scale=1.0, nwd_constant=12.7, eps=1e-7):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
    ...
    Args:
        ...
        xywh (bool, optional): If True, input boxes are in (x, y, w, h) format. If False, input boxes are in
                               (x1, y1, x2, y2) format. Defaults to True.
        feat_diag2 (torch.Tensor): A tensor representing the feature map w*d with shape (n, 1).
        ...
        MDPIoU1 (bool, optional): If True, calculate MDPIoU, raletive to c2. Defaults to False.
        MDPIoU2 (bool, optional): If True, calculate MDPIoU, raletive to feat2. Defaults to False.

        using_innerIoU (bool, optional): If True, using innerIoU replace IoU. Defaults to False.
        ratio (float, optional): ratio of innerIoU, [0.5, 1.5]. Defaults to 1.
        Focal (bool, optional): If True, using focal loss. Defaults to False.
        alpha (int, optional): alpha of alphaIoU . Defaults to 1.
        gamma (float, optional): gamma of SIoU. Defaults to 0.5.
        scale (bool, optional): If True, using WIoU. Defaults to False.
        eps (float, optional): A small value to avoid division by zero. Defaults to 1e-7.

    Returns:
        (torch.Tensor): IoU, GIoU, DIoU, CIoU, SIoU, EIoU, WIoU values depending on the specified flags.
    """
    ...
    if xywh:  # transform from xywh to xyxy
        ...
        w1, h1 = b1_x2 - b1_x1, b1_y2 - b1_y1 + eps
        w2, h2 = b2_x2 - b2_x1, b2_y2 - b2_y1 + eps
        w1_, h1_, w2_, h2_ = w1 / 2, h1 / 2, w2 / 2, h2 / 2  # for calculating innerIoU
        x1 = (b1_x1 + b1_x2) / 2
        y1 = (b1_y1 + b1_y2) / 2
        x2 = (b2_x1 + b2_x2) / 2
        y2 = (b2_y1 + b2_y2) / 2
    ...
    union = w1 * h1 + w2 * h2 - inter + eps
    if WIoU and scale:
        wiou_scale = WIoU_Scale(1 - (inter / union))
    ...
    if using_innerIoU:
        ...
        inner_inter = (torch.min(inner_b1_x2, inner_b2_x2) - torch.max(inner_b1_x1, inner_b2_x1)).clamp(0) * \
            (torch.min(inner_b1_y2, inner_b2_y2) -
                torch.max(inner_b1_y1, inner_b2_y1)).clamp(0)
        inner_union = w1*ratio * h1*ratio + w2 * \
            ratio * h2*ratio - inner_inter + eps
        inner_iou = inner_inter/inner_union
        iou = inner_iou
    if CIoU or DIoU or GIoU or EIoU or SIoU or WIoU or MDPIoU2 or ShapeIoU or NWD:
        # convex (smallest enclosing box) width
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
        if CIoU or DIoU or EIoU or SIoU or WIoU or MDPIoU2 or ShapeIoU or NWD:
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                if Focal:
                    ...
                    return getattr(WIoU_Scale, '_scaled_loss')(wiou_scale), (1 - iou) * torch.exp(
                        (rho2 / c2)), iou  # [scale, L_WIoUv1, iou] WIoU https://arxiv.org/abs/2301.10051
                else:
                    return iou, torch.exp((rho2 / c2))  # WIoU v1
            elif MDPIoU2:  # better on coco128
                d12 = (b2_x1 - b1_x1) ** 2 + (b2_y1 - b1_y1) ** 2
                d22 = (b2_x2 - b1_x2) ** 2 + (b2_y2 - b1_y2) ** 2
                if Focal:
                    return iou - ((d12 + d22) / c2), torch.pow(inter / (union + eps), gamma)
                else:
                    # MDPIoU https://arxiv.org/pdf/2307.07662.pdf  by AI&CV
                    return iou - ((d12 + d22) / c2)
            elif ShapeIoU:
                ww = 2 * torch.pow(w2, shape_iou_scale) / (torch.pow(w2, shape_iou_scale) + torch.pow(h2, shape_iou_scale))
                hh = 2 * torch.pow(h2, shape_iou_scale) / (torch.pow(w2, shape_iou_scale) + torch.pow(h2, shape_iou_scale))
                ...
                if Focal:
                    return iou - distance - 0.5 * (shape_cost), torch.pow(inter / (union + eps), gamma)
                else:
                    return iou - distance - 0.5 * (shape_cost)  # ShapeIoU https://arxiv.org/abs/2312.17663
            elif NWD:
                center_distance = rho2
                wh_distance = ((w1 - w2) ** 2 + (h1 - h2) ** 2) / 4
                wasserstein_2 = center_distance + wh_distance
                normalized_wasserstein = torch.exp(-torch.sqrt(wasserstein_2)/nwd_constant)
                if Focal:
                    return normalized_wasserstein, torch.pow(inter / (union + eps), gamma)
                else:
                    return normalized_wasserstein  # NWD https://arxiv.org/abs/2110.13389
            if Focal:
                # Focal_DIoU
    ...
    if MDPIoU1:
        ...
        d22 = (b2_x2 - b1_x2) ** 2 + (b2_y2 - b1_y2) ** 2
        if Focal:
            return iou - ((d12 + d22) / feat_diag2), torch.pow(inter / (union + eps), gamma)
        else:
            # MDPIoU https://arxiv.org/pdf/2307.07662.pdf  by AI&CV
            return iou - ((d12 + d22) / feat_diag2)
    if Focal:
...
def box_iou_for_nms(box1, box2, eps=1e-7):
